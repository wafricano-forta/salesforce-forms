/* =========================================================================
   ...
   Based on: in-home-app.js v3 (same SF field IDs, same hf-* form UI,
             ...
             - NEW step 3 question: preferred session time — sent in the
               standard Lead `description` field (searchable in SF).
   Hosted:   external script (jsDelivr) loaded via HTML Embed on the page.
   Testing:  window.__clwDryRun = true → submit → window.__clwLastPayload.
   v2 (Will, 7/14): contact-first step order; insurance state auto-picked
   from ZIP (range table) and user-overridable; hours capped at 40;
   "Billboard" lead source; consent links to the Electronic Comms Policy.
   v3 (Will, 7/14): In-Home cross-sell module + in-form In-Home interest
   question when the ZIP qualifies (checked against zip_code_coverage.json).
   v4 (Will, 7/16): OUTCOME ROUTING added. submit() now sets retURL + MQL
   Status from the answers the form already collects (no backend needed —
   Web-to-Lead still fires for every submission, so no leads are lost):
       Houston metro (770–775) -> /clinic/thank-you-schedule (MQL - Clinic)
       no insurance / payor DQ -> /clinic/thank-you-insurance
       no diagnosis            -> /clinic/thank-you-dx
   ...
   MQL Status is a write-in field, so values save as-is. The routing lives in
   computeClinicRoute() (just above submit()); only retURL + F_MQL_STATUS in
   submit() changed vs v3.
   ========================================================================= */
(function () {
  'use strict';
  ...
  // Preferred language — sent to the SF Language field (F_LANGUAGE).
  ...
  /* ZIP tier for the step-2 feedback note:
  ...
  /* Insurance state is per-insurance: a family can have a primary plan in one
     state and a secondary in another. Each defaults to the state derived from
     their ZIP; the "state" step is skipped by default and only surfaced
     per-plan via a "different state?" link. */
  ...
  /* ============================ FORM STATE ============================== */
  /* Step order: contact → ZIP+insurance → child → schedule → how-did-you-hear.
     Contact first converts best. */
  var state = {
    step: 1,
  ...
  /* ===================== OUTCOME ROUTING (v4) =========================== */
  /* Computed client-side from the answers the form already collects. Every
     submission still POSTs to Salesforce (capture everyone); this only decides
     the retURL page + MQL Status. First match wins.
     Precedence: insurance → diagnosis → clinic → in-home → virtual → DQ. */
  var TY = {
  ...
  /* Returns { url, mql }. Requested Service stays "In Clinic" (set in submit). */
  function computeClinicRoute() {
    ...
    if ((famState() === 'TX' || famState() === 'SC') && state.primaryType === 'medicaid')
  ...
  /* ======================= SALESFORCE SUBMIT ============================ */
  /* Every submission is captured as a lead. computeClinicRoute() sets retURL
     (which thank-you page the family lands on) and the MQL Status. Requested
     Service stays "In Clinic" (origin); MQL Status carries the routed service. */
  function submit() {
    if (recaptchaBlocks()) {
    ...
    var route = computeClinicRoute(); // ← v4: outcome routing (retURL + MQL Status)
    var fields = {};
    fields.oid = SF.OID;
    fields.retURL = route.url; // ← v4 (was: location.origin + location.pathname + '?submitted=1')
    ...
    fields[SF.F_MQL_STATUS] = route.mql; // ← v4 (was: '' intentionally blank)
